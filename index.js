...
// Paste this into an AWS Lambda function 


const invocationName = "worlds greatest example";

const languageStrings = {
   'en': {
        'translation': {
            'WELCOME1' : 'Welcome to worlds greatest example!',
            'WELCOME2' : 'Greetings!',
...
const handlers = {
    ...
    'AddDrinkIntent': function () {
        let say = 'Hello from AddDrinkIntent. ';

        var slotStatus = '';
        var resolvedSlot;

    //   SLOT: DrinkList 
        if (this.event.request.intent.slots.DrinkList.value) {
            const DrinkList = this.event.request.intent.slots.DrinkList;
            slotStatus += ' slot DrinkList was heard as ' + DrinkList.value + '. ';

            resolvedSlot = resolveCanonical(DrinkList);

            if(resolvedSlot != DrinkList.value) {
                slotStatus += ' which resolved to ' + resolvedSlot; 
            }
        } else {
            slotStatus += ' slot DrinkList is empty. ';
        }
